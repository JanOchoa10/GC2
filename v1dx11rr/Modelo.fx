...
PS_Input VS_Main(VS_Input vertex)
{
    ...
    PS_Input vsOut = (PS_Input) 0; vsOut.pos = mul(vertex.pos, worldMatrix);
    ...
    float3 c1 = cross(vsOut.normal, float3(0.0, 0.0, 0.1));
...
float4 PS_Main(PS_Input pix) : SV_TARGET
{
    ...
    color = float4(0.4, 0.4, 0.4, 1); // ambient color
    ...
    if (lightIntensity > 0)
    {
        ...
        color += (float4(0.5f, 0.5f, 0.5f, 1.0f) /*diffuse color*/ * lightIntensity);
