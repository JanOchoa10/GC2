...
//float miCambiador = 1.0f, miCambiadorTiempo = 0.2f, miCambiadorReinicio = 1.0f;
float limiteDeRotacionArriba = 0.0f;
float limiteDeRotacionAbajo = 0.0f;
		...
		if (gamePad->IsConnected())
		{
			...
			//if (dxrr->camaraTipo) {
				...
				if (grados > 0.19 || grados < -0.19) {
					dxrr->arriaba = grados / 15;
				}

				/*if (grados > 0.19 && limiteDeRotacionArriba < 5) {
					dxrr->arriaba = grados / 15;
					limiteDeRotacionArriba++;
					limiteDeRotacionAbajo--;
				}
				
				
				if (grados < -0.19 && limiteDeRotacionAbajo < 5) {
					dxrr->arriaba = grados / 15;
					limiteDeRotacionAbajo++;
					limiteDeRotacionArriba--;
				}*/
			...
			if (velocidad > 0.19 || velocidad < -0.19) {
				...
				else if (gamePad->GetState().Gamepad.wButtons & XINPUT_GAMEPAD_LEFT_SHOULDER) {
					velocidad /= 3;
				}
				else {
					velocidad *= 2.5;
			...
			/*if (gamePad->GetState().Gamepad.wButtons & XINPUT_GAMEPAD_A) {
				...
				miCambiador -= miCambiadorTiempo;

				if (dxrr->camaraTipo && miCambiador < 0) {
					dxrr->camaraTipo = false;
					miCambiador = miCambiadorReinicio;
				}
				else if (!dxrr->camaraTipo && miCambiador < 0)
				{
					dxrr->camaraTipo = true;
					miCambiador = miCambiadorReinicio;
				}
